...
if(isset($_POST['userType'])) {
	...
	if($userType >5 && $userType < 11) {
		...
		if(!$price) {
			echo "<script>swal('Oops...', '此类型的会员价格错误，请稍候重试！', 'error'); </script>";
		} elseif($okMoney < $price) {
			echo "<script>swal('Oops...', '当前可用余额不足完成此次交易！', 'error');</script>";
		} elseif($okMoney >=$price) {
			if(erphpSetUserMoneyXiaoFei($price)) {
				if(userPayMemberSetData($userType)) {
					...
					echo "<script>swal('Oops...', '系统发生错误，请联系管理员！', 'error');  </script>";
				}
			} else {
				echo "<script>swal('Oops...', '系统发生错误，请稍候重试！', 'error'); </script>";
			}
		} else {
			echo "<script>swal('Oops...', '未定义的操作！', 'error'); </script>";
		}
	} else {
		echo "<script>swal('Oops...', '会员类型错误！', 'error')</script>";
	}
}
if($_POST['action'] == 'card') {
	...
	if($result == '5') {
		echo "<script>swal('Oops...', '充值卡不存在！', 'error');</script>";
	} elseif($result == '0') {
		echo "<script>swal('Oops...', '充值卡已被使用！', 'error');</script>";
	} elseif($result == '2') {
		echo "<script>swal('Oops...', '充值卡密码错误！', 'error');</script>";
	} elseif($result == '1') {
		echo "<script>swal('恭喜','充值成功！', 'success');</script>";
	} else {
		echo "<script>swal('Oops...', '系统错误，请稍后重试！', 'error');</script>";
	}
	...
	if(is_numeric($epdmycrednum) && $epdmycrednum > 0 && get_option('erphp_mycred') == 'yes') {
		if(floatval(mycred_get_users_cred( $user_info->ID )) < floatval($epdmycrednum*get_option('erphp_to_mycred'))) {
			...
			if(addUserMoney($user_info->ID, $money)) {
				...
				$wpdb->query($sql);
				echo "<script>swal('恭喜','兑换成功！', 'success');</script>";
			} else {
				echo "<script>swal('Oops...','兑换失败！', 'error');</script>";
			}
		}
	}
}
...
<link rel='stylesheet' href="https://cdn.bootcdn.net/ajax/libs/limonte-sweetalert2/10.12.5/sweetalert2.min.css" type='text/css' media='all' />
        ...
        <div class="container">
          ...
          <div class="row boxmoe-user-page mt20">
            <div class="col-lg-12 col-md-12">
              <nav class="navbar navbar-expand-lg navbar-dark bg-user">
                <div class="container">
                  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbar-user" aria-controls="navbar-user" aria-expanded="false" aria-label="Toggle navigation">
                    ...
                    <div class="navbar-collapse-header">
                      <div class="row">
                        <div class="col-6 collapse-brand">
                          <span class="navbar-brand">会员中心菜单</span></div>
                        <div class="col-6 collapse-close">
                          <button type="button" class="navbar-toggler" data-toggle="collapse" data-target="#navbar-user" aria-controls="navbar-user" aria-expanded="false" aria-label="Toggle navigation">
                            <span></span>
                            <span></span>
                          </button>
                        </div>
                    ...
                    <div class="col-lg-4 col-md-4">
                      <div class="user-header card card-profile">
                        ...
                        <div class="card-body pt-0">
                          ...
                          <div class="row">
                            <div class="col">
                              <div class="card-profile-stats d-flex justify-content-center">
                                ...
                                <div>
                                  <span class="heading">
                                    <?php echo intval($userMoney->ice_get_money)?>
                                  </span>
                                  <span class="description">
                                    消费
                                  </span>
                          ...
                          <p class="control-label card-text">登陆用户：<?php echo esc_attr($user_info->user_login ); ?></p>
                          <p class="control-label card-text">会员等级：<?php boxmoe_vip_pic()?></p>
						  <p class="control-label card-text">到期时间：<?php $userTypeId=getUsreMemberType(); if($userTypeId>0 && $userTypeId<11){echo getUsreMemberTypeEndTime() ;}else{echo '当前未购买会员服务';}?></p>
                          <p class="control-label card-text">注册时间：<?php echo esc_attr( $user_info->user_registered ) ?></p>
                          <p class="control-label card-text">最近登录：<?php echo esc_attr( $user_info->last_login ) ?></p></div>
                      </div>
                    </div>
        ...
        if ($("#mm_name").val().trim().length == 0) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('.loader').remove();
					swal('恭喜','恭喜！修改信息成功', 'success');
                    $("#doprofile").val("保存");
                },
                error: function() {
                    $('.loader').remove();
					swal('Oops...','抱歉！修改失败！', 'error');
                    $("#doprofile").val("保存");
                }
            });
        }
        ...
        <?php
							if($list) {
								foreach($list as $value)
								{
									...
									echo "<td><a href=\"javascript:;\" data-fancybox=\"\" data-type=\"iframe\" data-src='".get_bloginfo('wpurl').'/wp-content/plugins/erphpdown/download.php?url='.$value->ice_url."' target='_blank'><span class='badge badge-pill badge-success'>下载页面</span></a></td>\n";
									echo "</tr>";
								}
							}
							else
...
$("#dopassword").click(function() {
        if ($("#mm_pass_new").val().trim().length == 0) {
			swal('Oops...','请输入新密码！', 'error');
        } else if (strlen($("#mm_pass_new").val().trim()) < 6) {
			swal('Oops...','密码长度至少为6位！', 'error');
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('.loader').remove();
					swal('恭喜','密码修改信息成功', 'success');
                    //alert(data);
                },
                error: function() {
                    $('.loader').remove();
                    swal('Oops...','密码修改失败！', 'error');
                }
            });
        }
    });	
